...
void AEnsEnemyBase::BeginPlay()
{
	...
	if (AbilitySystemComponent)
	{
		...
		FloatingInfosBarWidgetClass = StaticLoadClass(UObject::StaticClass(), nullptr,TEXT("/Game/Developers/Dorian/WB_FloatingInfoBar.WB_FloatingInfoBar_C"));
		if (!FloatingInfosBarWidgetClass)
			UE_LOG(LogTemp, Error, TEXT("%s() Failed to find WB_FloatingInfoBar. If it was moved, please update the reference location in C++."), *FString(__FUNCTION__));
	...
	HealthChangedDelegateHandle = AbilitySystemComponent->GetGameplayAttributeValueChangeDelegate(HealthAttributeSet->GetHealthAttribute()).
